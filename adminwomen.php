...
<p style="color:#FFF; font-size:48px; padding-top:10px; padding-left:5px;"> Admin Panel <span style="color:#FFF; font-size:15px;"> Add Women Dress</span> </p> </div>
<div class="container">
...
<label for="exampleInputEmail1">Dress Name </label>
  ...
  <div class="form-group">
<label for="exampleInputEmail1" style="margin-right: 50px;">Brand </label>
    <select name="selectedItem" class="dropdown" id="selectedItem" style="width:200px;">
...
<label for="exampleInputPassword1">Price</label>
  ...
  <label for="exampleInputPassword1">Fabric</label>
   <input type="text" name="fabric" id="color" class="form-control" placeholder="Fabric"   style="color:#000;"required>
   </div>
   <div class="form-group">
...
<label  class="checkbox-inline"> <input name="q1[]" type="checkbox" value="Extra Large">Extra Large </label> 
 ...
 <div class="form-group">
  ...
  $GetIDValue = mysqli_query($conn,$GetPreviousID);
    ...
    if (!file_exists('img/women/'.$LastID)) {
    mkdir('img/women/'.$LastID,0777,true);}

	   $brands = $_POST['selectedItem'];
	...
	$AA = 'img/women/'.$LastID.'/'.$LastID.' ('.$key.').'.$ext;
  if($key == 0){
    $Headerpic = $AA;
  }
	if($Ref1 == " ")
    ...
    if(move_uploaded_file($file_tmp, $AA)) {
			...
			$insertQry = "INSERT INTO women (dressname, price, brand, Details,headerpic ,pic_ref, size, fabric)
VALUES('$dreesname','$price','$brands','$details','$Headerpic','$Ref1','$q1', '$fabric')";
if(mysqli_query($conn,$insertQry)){

//Normalization for pic_ref
    $GetPreviousID1 = "Select id,pic_ref from women ORDER by ID DESC LIMIT 1";
...
$GetPreviousID2 = "Select id,size from women ORDER by ID DESC LIMIT 1";
...
for ($i=0; $i<count($words1); $i++){
  ...
  $ref1 = $words1[$i]; 
  if($ref1 != ""){
  $sql2 = "INSERT into table_size (id,size)
