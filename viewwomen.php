...
while($rows = mysqli_fetch_assoc($result))
{
	...
	echo '<td class="col-sm-1">' . $rows['dressname'] . '</td>';
	echo '<td class="col-sm-1">' . $rows['brand'] . '</td>';
	echo '<td class="col-sm-1">' . $rows['price'] . '</td>';
	
	echo '<td class="col-sm-1">' . $rows['size'] . '</td>';
	echo '<td class="col-sm-1">' . $rows['fabric'] . '</td>';
	
	echo '<td class="col-sm-1"> <a href="delete.php?id=' . $rows['id'] . '">Delete</a></td>';
	echo '</tr>';
